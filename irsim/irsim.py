...
# Decompiled from: Python 3.6.10 |Anaconda, Inc.| (default, May  8 2020, 02:54:21) 
# [GCC 7.3.0]
...
class IRSimModel:
    ...
    def load_ir_file(self, filename):
        ...
        if self.mainIp == -1:
            self.controller.log_error("Cannot find program entrance. Please make sure the 'main' function does exist")
        else:
    ...
    def step(self):
        if self.ip == -1:
            ...
            if error_code == 1:
                ...
                self.controller.console_info('Total instructions = %d', self.instCount)
                self.ip = -1
                self.pauseRunning = False
            else:
                if error_code == 2:
                    self.controller.log_error('An error occurred at line %d: Illegal memory access', self.ip + 1)
                    self.controller.console_error('An error occurred at line %d: Illegal memory access', self.ip + 1)
                    self.ip = -1
                    self.pauseRunning = False
                else:
                    if error_code == 3:
                        self.controller.log_warning('Program Counter goes out of bound')
                        self.controller.console_warning('Program Counter goes out of bound')
                        self.pauseRunning = False
                    else:
                        if error_code == 4:
                            self.controller.log_warning('Invalid input integer literal')
                            self.controller.console_warning('Invalid input integer literal')
                            self.pauseRunning = False
                        else:
                            self.ip += 1
    ...
    def sanity_check(self, inst, lineno):
        ...
        try:
                    ...
                    if tokens[0] == 'GOTO':
                        ...
                        if tokens[0] in ('RETURN', 'READ', 'WRITE', 'ARG', 'PARAM'):
                            ...
                            if tokens[0] == 'DEC':
                                ...
                                self.table_insert(tokens[1], int(tokens[2]), True)
                                ...
                                if tokens[0] == 'IF':
                                    ...
                                    self.codeList.append(('IF', tokens[1], tokens[2], tokens[3], tokens[5]))
                                else:
                                    ...
                                    if tokens[2] == 'CALL':
                                        if len(tokens) != 4:
                                            raise IRSyntaxError
                                        self.codeList.append(('CALL', tokens[0], tokens[3]))
                                    else:
                                        if len(tokens) == 3:
                                            self.table_insert(tokens[2])
                                            self.codeList.append(('MOV', tokens[0], tokens[2]))
                                        elif len(tokens) == 5:
            ...
            self.controller.log_error('Syntax error at line %d:\n%s', lineno + 1, inst)
        except DuplicatedLabelError:
            self.controller.log_error('Duplicated label %s at line %d:\n%s', tokens[1], lineno + 1, inst)
        except DuplicatedVariableError:
            self.controller.log_error('Duplicated variable %s at line %d:\n%s', tokens[1], lineno + 1, inst)
        except CurrentFunctionNoneError:
            self.controller.log_error('Line %d does not belong to any function:\n%s', lineno + 1, inst)
    ...
    def label_check(self):
        try:
            ...
            self.controller.log_error('Undefined label at line %d: %s', i, inst)
            return False
                    ...
                    if code[0] == 'GOTO':
                        ...
                        if code[0] == 'IF':
                            ...
                            if code[0] == 'MOV':
                                ...
                                if code[1][0] == '*':
                                    ptr = self.variableDict[code[1][1:]][0] // 4
                                    addr = self.memory[ptr] // 4
                                ...
                                if code[0] == 'ARITH':
                                    ...
                                    if code[0] == 'RETURN':
                                        ...
                                        if code[0] == 'CALL':
                                            ...
                                            if code[0] == 'ARG':
                                                self.argumentStack.append(self.get_value(code[1]))
                                            else:
                                                if code[0] == 'PARAM':
                                                    addr = self.variableDict[code[1]][0] // 4
...
class IRSimView(urwid.WidgetWrap):
    ...
    def instruction_view(self):
        if not hasattr(self, 'list_instruction'):
            ...
            self.list_instruction = urwid.ListBox(urwid.SimpleListWalker(instruction_list))
...
class IRSimController:
    ...
    def run(self):
        if non_interactive:
            self.model.run()
            return
        self.loop = urwid.MainLoop((self.view), (self.view.palette), unhandled_input=(self.exit_on_key))
        self.loop.run()
...
def init_logger(logpath):
    ...
    formatter = logging.Formatter('[%(levelname)s] %(asctime)s (%(filename)s %(lineno)s): %(msg)s')
...
def main():
    ...
    global non_interactive, debug_file, output_file
